...
This version is compatible with CAPME estimation
******************************************************************************/
...
void parametric2(double *SigmaInput_xx, double *SigmaInput_xy, int *m1, double *mu_input, double *lambdamin, int *nlambda, int *maxnlambda, double *iicov)
{
    ...
    for (i=0; i<m0; i++){
         for (j=0; j<m0; j++){
	        LMATRIX[i][j]      =  SigmaInput_xx[i*m0+j];
	        LMATRIX[i+m0][j]   = -SigmaInput_xx[i*m0+j];
		    LMATRIX[i][j+m0]   = -SigmaInput_xx[i*m0+j];
		    LMATRIX[i+m0][j+m0]=  SigmaInput_xx[i*m0+j];
            if(SigmaInput_xx[i*m0+j]!=0){
		      nz+=4;
	   	    }
	    }
    }
...
/* CAPME version */
    for(ColNum=0;ColNum<m0;ColNum++){
          MALLOC(        b, m,   double );  
          for (i=0;i<m0;i++){
	       b[i]=SigmaInput_xy[i*m0+ColNum];
	      }
          for (j=m0;j<m;j++){
           b[j]=-SigmaInput_xy[j*m0+ColNum];
          }
          solver2(m,n,nz,ia,ka,a,b,c);
	  FREE(b);
